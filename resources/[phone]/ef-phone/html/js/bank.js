...
$(document).on('click', '.bank-app-account', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Maze Bank", "Kontonummer kopieret!", "#8f0000", 1750);
});
    ...
    if (iban != "" && amount != "") {
            $.post('https://ef-phone/CanTransferMoney', JSON.stringify({
                ...
                if (data.TransferedMoney) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "Maze Bank", "Du har overført &#36; " + amount + "!", "#8f0000", 1500);

                } else {
                    QB.Phone.Notifications.Add("fas fa-university", "Maze Bank", "Du har ikke nok penge på kontoen!", "#8f0000", 1500);
                }
                QB.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        QB.Phone.Notifications.Add("fas fa-university", "Maze Bank", "Udfyld alle felter!", "#8f0000", 1750);
    }
});
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://ef-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                QB.Phone.Notifications.Add("fas fa-university", "Maze Bank", "Du har betalt &#36;" + InvoiceData.amount + "!", "#8f0000", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", "Maze Bank", "Der er ingen bankkonto knyttet til dette nummer!", "#8f0000", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});