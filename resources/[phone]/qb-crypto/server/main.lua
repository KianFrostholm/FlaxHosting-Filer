...
-- Registering a new command in Lua script
RegisterCommand('setcryptoworth', function(source, args, rawCommand)
    local src = source
    local user_id = vRP.getUserId({src})
    local crypto = tostring(args[1])
    
    if vRP.hasPermission({user_id, Crypto.Permission}) then
        if crypto and crypto ~= "" then
            if Crypto.Worth[crypto] then
                local NewWorth = tonumber(args[2])

                if NewWorth then
                    NewWorth = math.ceil(NewWorth)
                    ...
                    -- Update crypto worth and notify all clients
                    ...
                    TriggerClientEvent('QBCore:Notify', src, 'You have not provided a valid new value.')
                end
            else
                TriggerClientEvent('QBCore:Notify', src, 'This cryptocurrency does not exist.')
            end
        else
            TriggerClientEvent('QBCore:Notify', src, 'You have not provided a cryptocurrency.')
        end
    end
end)
...
-- Registering a new command in Lua script
RegisterCommand('checkcryptoworth', function(source, args, rawCommand)
    local src = source
    local user_id = vRP.getUserId({src})
    
    if vRP.hasPermission({user_id, Crypto.Permission}) then
        local crypto_value = Crypto.Worth['qbit']
        TriggerClientEvent('QBCore:Notify', src, 'The QBIT has a value of $' .. crypto_value)
    else
        TriggerClientEvent('QBCore:Notify', src, 'You do not have permission to check the crypto value.')
    end
end, false)


-- Registering a new command in Lua script
RegisterCommand('crypto', function(source, args, rawCommand)
    local src = source
    local user_id = vRP.getUserId({src})

    if user_id then
        if vRP.hasPermission({user_id, Crypto.Permission}) then
            local cryptoAmount = GetCrypto(user_id)
            local cryptoValue = Crypto.Worth['qbit']
            local MyPocket = math.ceil(cryptoAmount * cryptoValue)
        
            local message = Lang('text.you_have_with_a_value_of', {
                playerPlayerDataMoneyCrypto = cryptoAmount,
                mypocket = MyPocket
            })
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Player data could not be retrieved.')
    end
end, false)
...
RegisterServerEvent('qb-crypto:server:ExchangeFail', function()
    ...
    if vRP.tryGetInventoryItem({user_id, 'cryptostick', 1}) then
    else
        TriggerClientEvent('QBCore:Notify', src, 'error.cryptostick_malfunctioned', 'error')
    end
end)
...
RegisterServerEvent('qb-crypto:server:ExchangeSuccess', function(LuckChance)
    ...
    if vRP.tryGetInventoryItem({user_id, 'cryptostick', 1}) then
        ...
        AddCrypto(user_id, Amount)
        TriggerClientEvent('QBCore:Notify', src, Lang('success.you_have_exchanged_your_cryptostick_for', { amount = Amount }), 'success', 3500)
        TriggerClientEvent('qb-inventory:client:ItemBox', src, QBCore.Shared.Items['cryptostick'], 'remove')
        TriggerClientEvent('qb-phone:client:AddTransaction', src, Player, {}, Lang('credit.there_are_amount_credited', { amount = Amount }), 'Credit')
    end
end)

-- Callbacks

lib.callback.register('qb-crypto:server:HasSticky', function(src)
    local user_id = vRP.getUserId({src})

    if vRP.hasInventoryItem({user_id, 'cryptostick'}) then
        return true
    else
        return false
    end
...
lib.callback.register('qb-crypto:server:GetCryptoData', function(src, name)
    local user_id = vRP.getUserId({src})
    ...
    local CryptoData = {
        ...
        Portfolio = GetCrypto(user_id, 'GetCryptoData'),
        WalletId = i.registration..''..i.phone,
    }
...
lib.callback.register('qb-crypto:server:BuyCrypto', function(source, data)
    ...
    if bank >= total_price then
        local CryptoData = {
            History = Crypto.History['qbit'],
            Worth = Crypto.Worth['qbit'],
            Portfolio = GetCrypto(user_id) + tonumber(data.Coins),
            WalletId = i.registration..''..i.phone,
        }
        
        vRP.tryBankPayment({user_id, total_price})
        TriggerClientEvent('qb-phone:client:AddTransaction', source, user_id, data, Lang('credit.you_have_qbit_purchased', { dataCoins = tonumber(data.Coins) }), 'Credit')
        AddCrypto(user_id, tonumber(data.Coins))
...
lib.callback.register('qb-crypto:server:SellCrypto', function(source, data)
    ...
    if GetCrypto(user_id) >= tonumber(data.Coins) then
        local CryptoData = {
            History = Crypto.History['qbit'],
            Worth = Crypto.Worth['qbit'],
            Portfolio = GetCrypto(user_id) - tonumber(data.Coins),
        ...
        RemCrypto(user_id, tonumber(data.Coins))
        local amount = math.floor(tonumber(data.Coins) * tonumber(Crypto.Worth['qbit']))
        TriggerClientEvent('qb-phone:client:AddTransaction', source, Player, data, Lang('debit.you_have_sold', { dataCoins = tonumber(data.Coins) }), 'Debit')
...
lib.callback.register('qb-crypto:server:TransferCrypto', function(source, data)
    ...
    local i = MySQL.query.await('SELECT * FROM vrp_user_identities WHERE user_id = ?', {user_id})[1]
    if GetCrypto(user_id) >= tonumber(data.Coins) then
        local result = MySQL.query.await('SELECT * FROM `vrp_user_identities` WHERE `registration` LIKE ?', { data.WalletId })
        if result[1] ~= nil then
            local CryptoData = {
                History = Crypto.History['qbit'],
                Worth = Crypto.Worth['qbit'],
            ...
            if Target ~= nil then
                ...
                MySQL.update('UPDATE vrp_user_moneys SET money = ? WHERE user_id = ?', { json.encode(MoneyData), result[1].user_id })
            end
            return CryptoData
        else
            return 'notvalid'
...
function GetCrypto(user_id, event)
    --print('GetCrypto: ', user_id, event)
    local money = MySQL.query.await('SELECT * FROM vrp_user_moneys WHERE user_id = ?', {user_id})[1]

    return money.crypto
end

function AddCrypto(user_id, add)
    --print('AddCrypto: ', user_id)
    local money = MySQL.query.await('SELECT * FROM vrp_user_moneys WHERE user_id = ?', {user_id})[1]
    MySQL.Sync.execute('UPDATE vrp_user_moneys SET crypto = ? WHERE user_id = ?', {money.crypto + add, user_id})
end

function RemCrypto(user_id, rem)
    --print('RemCrypto: ', user_id)
    local money = MySQL.query.await('SELECT * FROM vrp_user_moneys WHERE user_id = ?', {user_id})[1]
    if money.crypto - rem >= 0 then
        MySQL.Sync.execute('UPDATE vrp_user_moneys SET crypto = ? WHERE user_id = ?', {money.crypto - rem, user_id})
        return true
    else
        return false
    end
