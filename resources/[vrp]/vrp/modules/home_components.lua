  ...
  wardrobe_enter = function(player,area)
    ...
    if user_id ~= nil and user_id == owner_id then
      ...
      if data.cloakroom_idle ~= nil then
        TriggerClientEvent("pNotify:SendNotification", player,{text = "Du har uniform på!", type = "error", queue = "global", timeout = 3000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
      end
        ...
        menu[lang.home.wardrobe.save.title()] = {function(player, choice)
          vRP.prompt(player, lang.home.wardrobe.save.prompt(), "", function(player, setname)
            ...
            if string.len(setname) > 0 then
              ...
              TriggerClientEvent("pNotify:SendNotification", player,{text = "Ugyldig værdi", type = "error", queue = "global", timeout = 3000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
            end
          end)
        end}
        ...
        vRP.prompt(player, lang.home.gametable.bet.prompt(), "", function(player, amount)
          ...
          if amount > 0 then
            if vRP.tryFullPayment(user_id,amount) then
              TriggerClientEvent("pNotify:SendNotification", player,{text = "Spil startet", type = "success", queue = "global", timeout = 3000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
              ...
              local close_bet = function()
                if bet_opened then
                  ...
                  if wuser_id ~= nil then
                    ...
                    TriggerClientEvent("pNotify:SendNotification", wplayer,{text = lang.money.received({bet_total}), type = "info", queue = "global", timeout = 3000, layout = "centerRight",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
                    ...
                    vRP.request(nplayer,lang.home.gametable.bet.request({amount}), 30, function(nplayer, ok)
                      if ok and bet_opened then
                        if vRP.tryPayment(nuser_id,amount) then -- register player bet
                          ...
                          TriggerClientEvent("pNotify:SendNotification", player,{text = "Betalte"..amount, type = "info", queue = "global", timeout = 3000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                        else
                          TriggerClientEvent("pNotify:SendNotification", player,{text = "Ikke nok penge", type = "error", queue = "global", timeout = 3000, layout = "centerRight",animation = {open = "gta_effects_fade_in", close = "gta_effects_fade_out"}})
                        end
                      end
