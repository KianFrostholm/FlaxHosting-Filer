...
$(function() {
    ...
    $(document).keyup(function(e) {
        if (e.key === "Escape") {
            $.post('https://rp-radio/close', JSON.stringify({}));
        };
        if (radioEdit) {
            if (e.key === "ArrowLeft") {
                if (radioWindow > 1) {
...
function ChangeFunc() {
    if (radioWindow === 1) {
        $.post('https://rp-radio/joinChannel', JSON.stringify({
            channel: $("#channel").val()
        }));
    } else if (radioWindow === 2) {
        $.post('https://rp-radio/changeInfo', JSON.stringify({
            type: "name",
            value: $("#channel2").val()
        }));
...
function OnOff() {
    if (radioOn) {
        ...
        $("#radiolist").empty();
        $("#radioimage").attr("src","images/" + radioColour + (radioOn ? "/on_" + radioType : "/off") + ".png");
        $.post('https://rp-radio/leaveChannel', JSON.stringify({}));
    } else {
        radioOn = true;
        $("#radioimage").attr("src","images/" + radioColour + (radioOn ? "/on_" + radioType : "/off") + ".png");
        $.post('https://rp-radio/radioOn', JSON.stringify({}));
        $(".screen-" + radioWindow).css("display", "block");
...
function VolUp() {
    $.post('https://rp-radio/VolUp', JSON.stringify({}));
    if (radioVolume < 100) {
...
function VolDown() {
    $.post('https://rp-radio/VolDown', JSON.stringify({}));
    if (radioVolume > 10) {
        radioVolume = radioVolume - 10
        $("#curvolume").html("Volume: " + radioVolume)
    }
