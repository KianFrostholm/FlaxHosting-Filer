...
function TchatGetMessageChannel (channel, cbr)
  local task = Task(cbr)
  MySQL.Async.fetchAll("SELECT * FROM phone_app_chat WHERE channel = @channel ORDER BY time DESC LIMIT 100", {channel = channel}, function(rows, affected)
    if #rows > 0 then
      task({rows})
    end
  end)
...
function TchatAddMessage (userid, channel, message)
  MySQL.Async.execute("INSERT INTO phone_app_chat (`userid`, `channel`, `message`) VALUES(@userid, @channel, @message); SELECT * from phone_app_chat WHERE `id` = (SELECT LAST_INSERT_ID());", {userid = userid, channel = channel, message = message}, function(rows, affected)
    if #rows > 0 then
