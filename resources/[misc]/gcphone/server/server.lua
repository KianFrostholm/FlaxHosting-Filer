...
function getMessages(phone, cbr)
    local task = Task(cbr)
    MySQL.Async.fetchAll("SELECT * FROM phone_messages WHERE receiver = @phone", {phone = phone}, function(rows, affected)
        if #rows > 0 then
            task({rows})
        else
            task()
    ...
    vRP.getUserIdentity({identifier, function(identity)
        if identity then
            ...
            if num ~= nil and mePhoneNumber ~= nil then
                MySQL.Async.execute("UPDATE phone_messages SET phone_messages.isRead = 1 WHERE phone_messages.receiver = @receiver AND phone_messages.transmitter = @transmitter", {receiver = mePhoneNumber, transmitter = num})
            end
        end
    end})
end
...
function deleteAllMessageFromPhoneNumber(identifier, phone)
    vRP.getUserIdentity({identifier, function(identity)
        if identity then
            local mePhoneNumber = identity.phone
            MySQL.Async.execute("DELETE FROM phone_messages WHERE receiver = @mePhoneNumber and transmitter = @phone", {mePhoneNumber = mePhoneNumber, phone = phone})
        end
    end})
end
...
function deleteAllMessage(identifier)
    vRP.getUserIdentity({identifier, function(identity)
        if identity then
            local mePhoneNumber = identity.phone
            MySQL.Async.execute("DELETE FROM phone_messages WHERE receiver = @mePhoneNumber", {mePhoneNumber = mePhoneNumber})
        end
    end})
end
...
function getHistoriqueCall (num, cbr)
    local task = Task(cbr)
    MySQL.Async.fetchAll("SELECT * FROM phone_calls WHERE phone_calls.owner = @num ORDER BY time DESC LIMIT 100", {num = num}, function(rows, affected)
        if #rows > 0 then
            task({rows})
        end
    end)
...
function saveAppels (appelInfo)
    if appelInfo.extraData == nil or appelInfo.extraData.useNumber == nil then
        MySQL.Async.execute("INSERT INTO phone_calls (`owner`, `num`,`incoming`, `accepts`) VALUES(@owner, @num, @incoming, @accepts)", {owner = appelInfo.transmitter_num, num = appelInfo.receiver_num, incoming = 1, accepts = appelInfo.is_accepts}, function(affected)
    ...
    if appelInfo.is_valid == true then
        ...
        MySQL.Async.execute("INSERT INTO phone_calls (`owner`, `num`,`incoming`, `accepts`) VALUES(@owner, @num, @incoming, @accepts)", {owner = appelInfo.receiver_num, num = num, incoming = 0, accepts = appelInfo.is_accepts}, function(affected)
...
AddEventHandler('gcPhone:appelsDeleteHistorique', function (numero)
    ...
    vRP.getUserIdentity({user_id, function(identity)
        if identity then
            local mePhoneNumber = identity.phone
            MySQL.Async.execute("DELETE FROM phone_calls WHERE `owner` = @owner AND `num` = @num", {owner = mePhoneNumber, num = numero})
        end
    end})
...
function appelsDeleteAllHistorique(srcIdentifier)
    vRP.getUserIdentity({srcIdentifier, function(identity)
        if identity then
            local mePhoneNumber = identity.phone
            MySQL.Async.execute("DELETE FROM phone_calls WHERE `owner` = @owner", {owner = mePhoneNumber})
        end
    end})
end
...
AddEventHandler('onResourceStart', function(name)
    if name:lower() == GetCurrentResourceName():lower() then
        Wait(5000)
        MySQL.Async.fetchAll("SELECT name,message,time FROM `phone_twitter` ORDER by id DESC LIMIT 100", {}, function(rows, affected)
            if #rows > 0 then
